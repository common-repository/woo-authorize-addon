...
if( ! class_exists( 'WOO_WCAUTHNET_CIM_API' ) ){
	/**
	 * WooCommerce Authorize.net CIM API handler class
	 *
	 * @since 1.0.0
	 */
	class WOO_WCAUTHNET_CIM_API {
		...
		public function do_request( $xml ){
			...
			$wp_http_args = array(
				'timeout'     => apply_filters( 'woo_wcauthnet_cim_api_timeout', 45 ),
				'redirection' => 0,
				'httpversion' => '1.0',
				'sslverify'   => false,
				'blocking'    => true,
		...
		public function get_create_payment_transaction_xml( $order, $payment_details, $transaction_mode = 'authCaptureTransaction' ) {
			...
			$this->xml_writer->writeElement( 'invoiceNumber', waa_get_order_id( $order ) );
			...
			if ( $billing_email = waa_get_prop( $order, 'billing_email', true ) ) {
			...
			if( waa_get_prop( $order, 'shipping_country', true ) ){
				// <shipTo>
		...
		public function get_update_customer_profile_xml( $order, $customer_profile_id ) {
			...
			$this->xml_writer->writeElement( 'email', is_null( $order ) ? $user_email : waa_get_prop( $order, 'billing_email', true ) );
			$this->xml_writer->writeElement( 'customerProfileId', $customer_profile_id );
		...
		protected function add_address_xml( $order, $type = 'billing', $fields = array() ) {
			...
			foreach ( $fields as $field ) {
				...
				if ( 'address' == $field ) {
					$this->xml_writer->writeElement( 'address', waa_get_prop( $order, $type . '_address_1', true ) . ' ' . waa_get_prop( $order, $type . '_address_2', true ) );
					continue;
				}

				if( 'zip' == $field ){
					$this->xml_writer->writeElement( 'zip', waa_get_prop( $order, $type . '_postcode', true ) );
					continue;
				}

				if( 'phone' == $field ){
					$this->xml_writer->writeElement( 'phoneNumber', waa_get_prop( $order, 'billing_phone', true ) );
					continue;
				}

				if( 'country' == $field ){
					$this->xml_writer->writeElement( 'country', $this->convert_country_code( waa_get_prop( $order, $type . '_country', true ) ) );
					continue;
				}

				if ( $field_value = waa_get_prop( $order, $field_name, true ) ) {
		...
		protected function add_line_items_xml( $order ){
			...
			if( ! empty( $order_items ) ){
				foreach( $order_items as $item_id => $item ){
					...
					$this->xml_writer->writeElement( 'itemId', apply_filters('woo_wcauthnet_add_line_item_id',$item_id,$item,$order ));
					// <name>Item name</name>
		...
		protected function add_tax_xml( $order ) {
			...
			// <name>
			$this->xml_writer->writeElement( 'name', __( 'Taxes', 'woo-woocommerce-authorizenet-payment-gateway' ) );
		...
		protected function add_shipping_xml( $order ) {
			...
			// <name>
			$this->xml_writer->writeElement( 'name', __( 'Shipping', 'woo-woocommerce-authorizenet-payment-gateway' ) );
		...
		protected function add_user_info( $order ){
			$user_info = apply_filters( 'woo_wcauthnet_user_info', array(
				'transaction_email' => waa_get_prop( $order, 'billing_email', true ),
...
/**
 * Unique access to instance of WOO_WCAUTHNET_CIM_API class
 *
 * @return \WOO_WCAUTHNET_CIM_API
 * @since 1.0.0
 */
function WOO_WCAUTHNET_CIM_API(){
	return WOO_WCAUTHNET_CIM_API::get_instance();
}