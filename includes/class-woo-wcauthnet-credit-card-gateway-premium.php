...
if( ! class_exists( 'WOO_WCAUTHNET_Credit_Card_Gateway_Premium' ) ) {
	/**
	 * WooCommerce Authorize.net gateway class
	 *
	 * @since 1.0.0
	 */
	class WOO_WCAUTHNET_Credit_Card_Gateway_Premium extends WOO_WCAUTHNET_Credit_Card_Gateway {
		 ...
		 *
		 * @var \WOO_WCAUTHNET_Credit_Card_Gateway_Premium
		 * @since 1.0.0
		 */
		 ...
		 *
		 * @return \WOO_WCAUTHNET_Credit_Card_Gateway_Premium
		 * @since 1.0.0
		 */
		...
		/**
		 * Create unique instance of WOO_WCAUTHNET_Credit_Card_Gateway_Premium class
		 *
		 * @return \WOO_WCAUTHNET_Credit_Card_Gateway_Premium Unique instance
		 * @since 1.0.0
		 */
		public function __construct() {
			...
			$this->api = new WOO_WCAUTHNET_CIM_API();
			...
			if( 'redirect' == $this->payment_method ){
				add_action( 'woocommerce_receipt_woo_wcauthnet_gateway', array( $this, 'print_authorize_net_payment_form' ), 10, 1 );
			}
		}
		...
		public function add_advanced_options() {
			...
			$premium_options = array(
				'payment_method' => array(
					'title' => __( 'Request mode', 'woo-woocommerce-authorizenet-payment-gateway' ),
					'type' => 'select',
					'description' => __( 'The option "API" will allow using the potentialities of the APIs to complete the purchase without external redirects.', 'woo-woocommerce-authorizenet-payment-gateway' ),
					'options' => array(
						'api' => __( 'API', 'woo-woocommerce-authorizenet-payment-gateway' )
					),
                    'class'   => 'api_type wc-enhanced-select-api',
					'default' => 'api'
				),
				'cim_handling' => array(
					'title' => __( 'Enable Customer Information Manager (CIM)', 'woo-woocommerce-authorizenet-payment-gateway' ),
					'type' => 'checkbox',
					'description' => __( 'It allows the system to store the payment methods of the users, in order to use them in the future without the need to write the details again.', 'woo-woocommerce-authorizenet-payment-gateway' ),
					'default' => 'yes'
				),
				'itemized' => array(
					'title' => __( 'Enable itemized transaction', 'woo-woocommerce-authorizenet-payment-gateway' ),
					'type' => 'checkbox',
					'description' => __( 'Enable the registration of the items in the cart during the transaction (up to a maximum of 30 items)', 'woo-woocommerce-authorizenet-payment-gateway' ),
					'default' => 'yes'
				),
		...
		public function process_refund( $order_id, $amount = null, $reason = '' ){
			...
			if( ! $order || ! $this->can_refund_order( $order ) ){
				...
				$order->add_order_note( __( 'Authorize.net refund failed: No transaction ID', 'woo-woocommerce-authorizenet-payment-gateway' ) );
				return false;
			}
			...
			if( ! empty( $response ) ){
				...
				if( 1 == $transaction_status ){
					...
					$order->add_order_note( __( 'Authorize.net refund approved', 'woo-woocommerce-authorizenet-payment-gateway' ) );
				}
				else{
					$response_message = '';
					...
					if ( 'yes' == $this->debug ) {
						$this->log->add( 'authorize.net', sprintf( __( 'Refund error: %s', 'woo-woocommerce-authorizenet-payment-gateway' ), $response_message ) );
					}
					$order->add_order_note( sprintf( __( 'Refund error: %s', 'woo-woocommerce-authorizenet-payment-gateway' ), $response_message ) );
					return false;
				}
			}
			else{
				if ( 'yes' == $this->debug ) {
					$this->log->add( 'authorize.net', __( 'Refund error: unknown error', 'woo-woocommerce-authorizenet-payment-gateway' ) );
				}
				$order->add_order_note( __( 'Refund error: unknown error', 'woo-woocommerce-authorizenet-payment-gateway' ) );
				return false;
			}
		...
		public function payment_fields() {
			...
			if( 'api' == $this->payment_method ) {
				echo '<div id="woo_wcauthnet_credit_card_form">';
		...
		protected function _process_api_payment( $order ) {
			$order_id = waa_get_order_id( $order );
			$payment = $this->_get_payment_details();

			if( 'credit_card' == $payment->type && ( empty( $payment->card_number ) || empty( $payment->expiration_date ) ) ){
				wc_add_notice( __( 'Missing required information', 'woo-woocommerce-authorizenet-payment-gateway' ), 'error' );
				return;
			}
			...
			if ( ! empty( $response ) ){
					...
					if ( 1 == $transaction_status ) {
						if ( $order->has_status( 'completed' ) ) {
							...
							if ( $order->get_total() != $transaction_amount ) {
								...
								$order->update_status( 'on-hold', sprintf( __( 'Validation error: Authorize.net amounts do not match with (%s).', 'woo-woocommerce-authorizenet-payment-gateway' ), $transaction_amount ) );

								wc_add_notice( sprintf( __( 'Validation error: Authorize.net amounts do not match with (%s).', 'woo-woocommerce-authorizenet-payment-gateway' ), $transaction_amount ), 'error' );

								return;
							}

							// Validate Email Address
							$billing_email = waa_get_prop( $order, 'billing_email', true );
							...
							if (strcasecmp( trim( $billing_email ), trim( $transaction_email ) ) != 0 ) {
								...
								// Put this order on-hold for manual checking
								$order->update_status( 'on-hold', sprintf( __( 'Validation error: Authorize.net responses from a different email address than (%s).', 'woo-woocommerce-authorizenet-payment-gateway' ), $transaction_email ) );

								wc_add_notice( sprintf( __( 'Validation error: Authorize.net responses from a different email address than (%s).', 'woo-woocommerce-authorizenet-payment-gateway' ), $transaction_email ), 'error' );

								return;
							}

							// Mark as complete
							$order->add_order_note( sprintf( __( 'Authorize.net payment completed (message: %s). Transaction ID: %s', 'woo-woocommerce-authorizenet-payment-gateway' ), $transaction_message, $transaction_id ) );
							$order->payment_complete( $transaction_id );

							if ( ! empty( $transaction_account_num ) ) {
								waa_save_prop( $order, 'x_card_num', wc_clean( $transaction_account_num ) );
								waa_save_prop( $order, 'x_card_expiration_date', wc_clean( $payment->expiration_date ) );
							}
							...
							if ( $this->cim_handling == 'yes' && ! empty( $user_id ) ) {
								...
								if( ! empty( $payment_profile ) ){
									...
									// checks for error E00039 (duplicated payment profile)
									if( ! isset( $response->messages ) || ! isset( $response->messages->message ) || ! isset( $response->messages->message->code ) || $response->messages->message->code != 'E00039' ){
                        ...
                        $response_message = apply_filters('woo_wcauthnet_response_message',$response_message,$transaction_status);

						wc_add_notice( sprintf( __( 'Payment error: %s', 'woo-woocommerce-authorizenet-payment-gateway' ), $response_message ), 'error' );
				...
				wc_add_notice( __( 'An error occurred while processing the payment; please try again later', 'woo-woocommerce-authorizenet-payment-gateway' ), 'error' );
				return;
			}
		}
		...
		protected function _get_payment_details(){
			...
			$card_id = isset( $_POST['wc-woo_wcauthnet_credit_card_gateway-payment-token'] ) ? $_POST['wc-woo_wcauthnet_credit_card_gateway-payment-token'] : false;
		...
		protected function _get_refund_details( $order ){
			...
			$payment_obj->type = "credit_card";
			$payment_obj->card_number = waa_get_prop( $order, 'x_card_num', true );
			$payment_obj->expiration_date = waa_get_prop( $order, 'x_card_expiration_date', true );
...
/**
 * Unique access to instance of WOO_WCAUTHNET_Credit_Card_Gateway_Premium class
 *
 * @return \WOO_WCAUTHNET_Credit_Card_Gateway_Premium
 * @since 1.0.0
 */
function WOO_WCAUTHNET_Credit_Card_Gateway_Premium(){
	return WOO_WCAUTHNET_Credit_Card_Gateway_Premium::get_instance();
}