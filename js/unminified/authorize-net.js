jQuery( document ).ready( function( $ ){
    var payment_profile,
        ...
        routing_number,
        account_number,
        credit_card_form = $( '#woo_wcauthnet_credit_card_form'),
        echeck_form = $( 'div.payment_method_woo_wcauthnet_echeck_gateway'),
        ...
        radio_change_handling = function(){
            if( credit_card_form.length != 0 ){
                credit_card_form.find( '.payment-profile-radio' ).change( function(){
                    ...
                    if( t.is( '#woo_wcauthnet_payment_profile_new:checked' ) ){
                        new_form.slideDown();
                    }
                    else{
                        new_form.slideUp();
    ...
    $( 'body' ).on( 'updated_checkout', function(){
        ...
        var save_form_values = function(){
                ...
                echeck_form
                    .on( 'change', '#woo_wcauthnet_echeck_gateway-routing-number', function(){ routing_number = $(this).val() } )
                    .on( 'change', '#woo_wcauthnet_echeck_gateway-account-number', function(){ account_number = $(this).val() } );
            },
            restore_form_values = function(){
                ...
                echeck_form
                    .find( '#woo_wcauthnet_echeck_gateway-routing-number').val( routing_number).end()
                    .find( '#woo_wcauthnet_echeck_gateway-account-number').val( account_number );
            };
